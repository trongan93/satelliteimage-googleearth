...
def downloadBestRGBImages(satelliteImages, lat, lng, img_region, object_id, error_query):
    ...
    rectangle_30m_per_pixel = img_region[0]
...
def downloadLandslideImageFilesToLocal(objectid, urls_obj):
    # print(objectid)
    # print(urls_obj)
    saved_paths = []
...
def downloadNonLandslideImageFilesToLocal(objectid, urls_obj):
    # print(objectid)
    # print(urls_obj)
    saved_paths = []
    for i in range(0, len(urls_obj), 2):
        ...
        if url != '':
            try:
                ...
                raise SystemExit(e)
            # filename = getFilename_fromCd(r.headers.get('content-disposition'))
            saved_path = "{}/{}/{}/Negative".format(fileconfig.base_saved_data_path, objectid, satellite)
            if not os.path.exists(saved_path):
                os.makedirs(saved_path)
            saved_path_file = "{}/downloaded.zip".format(saved_path)
            open(saved_path_file,'wb').write(r.content)
